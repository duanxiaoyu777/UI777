...
class PySelenium(object):
    ...
    def get_province(self, css):
        """获取地址弹窗顶级元素，判断displayed是否为空"""
        province = self.get_elements(css)
        for i in province:
            if i.is_displayed():
                # print(i)
                return i

    def click_province(self, css1, css2, city):
        """封装省/市/区/街道"""
        top_w = self.get_province(css1)
        ele_1 = top_w.find_elements_by_xpath(css2)
        for i in ele_1:
            if i.text == city:
                i.click()

    def get_date_picker(self, css):
        """获取日期选择器元素，判断displayed是否为空"""
        date_picker = self.get_elements(css)
        for i in date_picker:
            if i.is_displayed():
                # print(i)
                return i

    def click_date_picker(self, css1, css2, date):
        """封装日期选择器 """
        top_q = self.get_date_picker(css1)
        ele = top_q.find_elements_by_xpath(css2)
        for i in ele:
            if i.text == date:
                i.click()
